...
export function RichTextField({
  ...
  const linkSelection = useRef<TextSelection>({ start: 0, end: 0 })

  function apply(transform: (value: string, sel: TextSelection) => { value: string; selection: TextSelection }) {
    const el = ref.current
    if (!el) return
    const sel: TextSelection = { start: el.selectionStart ?? value.length, end: el.selectionEnd ?? value.length }
    ...
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <ToolButton title="Negrito" onClick={() => apply((v, s) => wrapSelection(v, s, '**', '**'))}>
            <Bold size={13} />
          </ToolButton>
          <ToolButton title="Itálico" onClick={() => apply((v, s) => wrapSelection(v, s, '*', '*'))}>
            <Italic size={13} />
          </ToolButton>
          <ToolButton title="Sublinhado" onClick={() => apply((v, s) => wrapSelection(v, s, '__', '__'))}>
            <Underline size={13} />
          </ToolButton>
          <ToolButton title="Rasurado" onClick={() => apply((v, s) => wrapSelection(v, s, '~~', '~~'))}>
