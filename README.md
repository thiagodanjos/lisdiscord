...
Os comandos `/movcall`, `/movhoras`, `/pontosmovadmin`, `/resetmovcall`, `/inativos` e `/sorteio` só aparecem, por omissão, para membros com a permissão **Gerir servidor** — ajustável em **Definições do servidor → Integrações** no Discord. `/pontosmov` e `/help` ficam disponíveis para toda a gente.
